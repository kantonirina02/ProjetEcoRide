 ...
 */
final class Version20251201171911 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE reviews (id INT AUTO_INCREMENT NOT NULL, author_id INT NOT NULL, target_id INT NOT NULL, ride_id INT DEFAULT NULL, validated_by_id INT DEFAULT NULL, rating INT NOT NULL, comment LONGTEXT DEFAULT NULL, created_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime_immutable)\', status VARCHAR(20) DEFAULT \'pending\' NOT NULL, validated_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', moderation_note LONGTEXT DEFAULT NULL, INDEX IDX_6970EB0FF675F31B (author_id), INDEX IDX_6970EB0F158E0B66 (target_id), INDEX IDX_6970EB0F302A8A70 (ride_id), INDEX IDX_6970EB0FC69DE5E5 (validated_by_id), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        $this->addSql('CREATE TABLE ride_participants (id INT AUTO_INCREMENT NOT NULL, ride_id INT NOT NULL, user_id INT NOT NULL, seats_booked INT NOT NULL, credits_used INT NOT NULL, status VARCHAR(20) NOT NULL, requested_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime_immutable)\', confirmed_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', cancelled_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', feedback_status VARCHAR(20) DEFAULT \'pending\' NOT NULL, feedback_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', feedback_note LONGTEXT DEFAULT NULL, INDEX IDX_7927B92D302A8A70 (ride_id), INDEX IDX_7927B92DA76ED395 (user_id), INDEX idx_rp_status (status), UNIQUE INDEX uniq_ride_user (ride_id, user_id), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        $this->addSql('CREATE TABLE rides (id INT AUTO_INCREMENT NOT NULL, driver_id INT NOT NULL, vehicle_id INT NOT NULL, from_city VARCHAR(120) NOT NULL, to_city VARCHAR(120) NOT NULL, start_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime_immutable)\', end_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', price NUMERIC(8, 2) NOT NULL, seats_total INT NOT NULL, seats_left INT NOT NULL, status VARCHAR(20) NOT NULL, allow_smoker TINYINT(1) DEFAULT 0 NOT NULL, allow_animals TINYINT(1) DEFAULT 0 NOT NULL, music_style VARCHAR(50) DEFAULT NULL, created_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime_immutable)\', updated_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', payout_released_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', INDEX IDX_9D4620A3C3423909 (driver_id), INDEX IDX_9D4620A3545317D1 (vehicle_id), INDEX idx_rides_from_to_start (from_city, to_city, start_at), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        ...
        $this->addSql('CREATE TABLE users (id INT AUTO_INCREMENT NOT NULL, email VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, pseudo VARCHAR(255) NOT NULL, phone VARCHAR(255) DEFAULT NULL, credits_balance INT DEFAULT 0 NOT NULL, created_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime_immutable)\', roles JSON NOT NULL, driver_preferences JSON DEFAULT NULL, suspended_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', suspension_reason VARCHAR(255) DEFAULT NULL, profile_photo LONGTEXT DEFAULT NULL, UNIQUE INDEX uniq_user_email (email), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        ...
        $this->addSql('ALTER TABLE reviews ADD CONSTRAINT FK_6970EB0FF675F31B FOREIGN KEY (author_id) REFERENCES users (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE reviews ADD CONSTRAINT FK_6970EB0F158E0B66 FOREIGN KEY (target_id) REFERENCES users (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE reviews ADD CONSTRAINT FK_6970EB0F302A8A70 FOREIGN KEY (ride_id) REFERENCES rides (id) ON DELETE SET NULL');
